...
public class HeartService extends Service {
    ...
    public class TCPHeartClientTask extends Thread {
        ...
        public void run() {
            try {
                ...
                boolean connected = socketChannel.connect(new InetSocketAddress("0.0.0.0", 5660));
                if(connected) {
                ...
                while(true) {
                    ...
                    if(pendantDevice.udp.selector.select(7000) > 0) {
                        ...
                        while (buffer.hasRemaining()) {
                            cmd += new String(new byte[]{buffer.get()}, StandardCharsets.UTF_8);
                        }
                        process_cmd(cmd);
                    }
    ...
    public void sendUDPCmd(String cmd) {
        try {
            ...
            pendantDevice.udp.channel.send(getUDPCmd(cmd), new InetSocketAddress("0.0.0.0", 5600));
    ...
    public void sendUDPCmd(ByteBuffer cmd) {
        try {
            pendantDevice.udp.channel.send(cmd, new InetSocketAddress("0.0.0.0", 5600));
            pendantDevice.udp.last_write_time = LocalDateTime.now();
        } catch (IOException e) {
    ...
    public void initBleClient() {
        ...
        if(pendantAddress != null) {
            ...
            companionDeviceManager.associate(request, getMainExecutor(), new CompanionDeviceManager.Callback() {
